...
class ConvertingValues: ConvertingValuesProtocol & ConvertingMethodsProtocol//, ObservableObject
{
    ...
    func addSymbolToPrimaryValue(_ tag: BoardKeysTags) -> (Bool, String) {
        ...
        guard
            (indexOfDelimiter < 0 && tag == Constants.delimiter)
                || (tag != Constants.delimiter)
        else { return falseResult }
        guard
            (((characters.count - indexOfDelimiter) <= 2)
                && (tag != Constants.delimiter)) || ((indexOfDelimiter < 0))
        else { return falseResult }
        guard
            characters.count
                < Constants.FavoriteCurrencyTableView.primaryMaxChars
        else { return falseResult }
        let result = str + tag.rawValue
        if indexOfDelimiter == 1
            || (result.count <= 2 && indexOfDelimiter < 0
                && tag == Constants.delimiter)
        {
            list[0].value = result
        } else {
    ...
    func getCurrencyValue(code: String) -> String {
        guard let item = list.first(where: { $0.code == code }) else {
            return ""
        }
        return item.value
    }
